...
說明:
區分為三個步驟
1. 整合過去資料庫中的所有資料，並篩選出昆蟲類、兩爬類以及魚貝類資料
2. 整合數化資料，並檢查資料是否有錯誤
3, 將兩者資料整合並網格化
...
#### 移除其他多餘的資訊
colnames(d_rp_na)
d_rp_na_2 <- d_rp_na[,c(43:53,55:59,25,33)]
...
colnames(name_check)
rdt_all <- rdt_all[name_check,on=.(`目名`,`科名(原始報告)`,`科名`,`物種俗名(原始報告)`,`物種學名(原始報告)`,`物種學名`)]
rdt_all_add <- rdt_all[d_rp_na_2,on=.(sp_id=id)]
rm(name_check,name_check_3,name_check_4,rdt_all_add_name)
### 處理數化好的資料

rdt_all_add[,c("綱","目名","科名","物種學名","分類階層"):=
              .(i.綱,i.目名,i.科名,i.物種學名,level)]
rdt_all_add[,c(25:41):=NULL]
write.csv(rdt_all_add,paste0(data_path,"/result/database_insect_數化資料.csv"))
####資料清除
rm(d_rp_na,d_rp_na_2,name_l_art,rdt,rdt_all_add,sp_n,rdt_all)
```
